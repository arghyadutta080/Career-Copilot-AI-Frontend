...
export function ReviewStep({
  ...
  const handleStart = async () => {
    ...
    try {
      // 1. Create analysis record (quota check happens server-side before this succeeds)
      ...
      // Detect quota-exceeded error from the server (HTTP 403 + code: FREE_LIMIT_REACHED)
      if (err?.message?.includes("analysis limit")) {
    ...
    <>
      <Card className="max-w-2xl mx-auto space-y-6">
        <div className="text-center py-6">
          <div className="inline-flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-violet-600 to-indigo-600 shadow-[0_0_30px_rgba(37,99,235,0.3)] text-white mb-6">
        ...
        <div className="flex justify-between pt-6 border-t border-zinc-800/60 mt-8">
          ...
          <button
            id="review-step-start-btn"
            onClick={handleStart}
            disabled={starting}
            className="flex items-center gap-2 rounded-xl bg-violet-600 px-8 py-2.5 text-sm font-medium text-white hover:bg-violet-500 transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-[0_0_20px_rgba(37,99,235,0.3)] hover:shadow-[0_0_25px_rgba(37,99,235,0.5)]"
          >
            {starting ? (
              <>
      ...
      {/* Upgrade modal — shown when the quota is exceeded */}
      {showUpgradeModal && (
